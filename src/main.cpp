...
unsigned char * toRGB_Mat(Mat & image) {
	unsigned char* in = new unsigned char[image.cols*image.rows * image.channels()];
	int ch = image.channels();
	for (int y = 0; y < image.rows; y++) {
		unsigned char* data = image.ptr<unsigned char>(y);
		for (int x = 0; x < image.cols; x++) {
			int idx = image.cols*ch*y + x*ch;
			in[idx] = data[x*ch];
			in[idx + 1] = data[x*ch+1];
			in[idx + 2] = data[x*ch+2];
		}
	}
	return in;
}
...
void toIMG_mat(unsigned char* in, Mat &image) {
	int ch = image.channels();
	for (int i = 0; i < image.rows; i++) {
		unsigned char *data = image.ptr<unsigned char>(i);
		for (int j = 0; j < image.cols; j++) {
			for (int k = 0; k < ch; k++) {
				data[j*ch + k] = in[image.cols*ch*i + j*ch + k];
			}
		}
	}
}
...
int main() {
 ...
 // CImg<unsigned char> images[5];
  Mat images[5];
  images[0] = imread("img/hanoi1.png");
  images[1] = imread("img/hanoi2.png");
  images[2] = imread("img/hanoi3.png");
  images[3] = imread("img/hanoi4.png");
  images[4] = imread("img/hanoi5.png");
...
//  images[4] = CImg<unsigned char>("img/hanoi5.png");
 ...
 // int height = images[0].height();
  ...
  //pipline 1.1 grayscale conversion
  for (int i = 0; i < 5; i++) {
